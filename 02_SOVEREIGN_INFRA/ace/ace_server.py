...
class CMSKnowledgeBridge:
    ...
    def sync_rules(t_dir):
        ...
        rules_content = """# Evolution Context Engine (ACE) - Dynamic Rules
        
## 🧠 Invariantes de Governança (Lidas via ACE)
Você está operando sob o escudo de governança do Evolution CMS. Não invente nada.
...
"""
        try:
            ...
            rules_path = os.path.join(cursor_dir, "evolution_core.mdc")
            with open(rules_path, "w", encoding="utf-8") as f:
                f.write("---\ndescription: Core invariants and rules for the Evolution system.\nglobs: *\n---\n" + rules_content)
...
class ACEEventHandler(FileSystemEventHandler):
    def on_modified(self, event):
        ...
        if status == "running" and not event.is_directory:
            ...
            if ".cursor" not in path and ".git" not in path and "node_modules" not in path and not path.endswith(".clinerules") and not path.endswith("evolution_core.mdc"):
